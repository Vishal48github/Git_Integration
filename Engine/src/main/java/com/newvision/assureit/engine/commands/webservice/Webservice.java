 ...
 */
package com.newvision.assureit.engine.commands.webservice;

import com.newvision.assureit.engine.commands.*;
...
public class Webservice extends General {
    ...
    @Action(object = ObjectType.WEBSERVICE, desc = "PATCH Soap Request ", input = InputType.YES, condition = InputType.OPTIONAL)
    public void patchSoapRequest() {
        createhttpRequest(RequestMethod.PATCH);
    }
    ...
    private String handleDataSheetVariables(String payloadstring) {
        ...
        for (int sheet = 0; sheet < sheetlist.size(); sheet++) {
            if (payloadstring.contains("{" + sheetlist.get(sheet) + ":")) {
                com.newvision.assureit.datalib.testdata.model.TestDataModel tdModel = Control.getCurrentProject()
    ...
    private void createhttpRequest(RequestMethod requestmethod) {
        try {
            ...
            } else if(requestmethod.toString().equals("PATCH")){
              createPatchHttprequest();
              return;
            }
   ...
   private void createPatchHttprequest() {
        try (CloseableHttpClient httpClient = HttpClients.createDefault()) {            
        
            String url = httpConnections.get(key).getURL().toString();            
            ...
            try (CloseableHttpResponse response = httpClient.execute(httpPatch)) {                
                ...
                responsecodes.put(key, Integer.toString(response.getStatusLine().getStatusCode()));
                responsemessages.put(key, httpConnections.get(key).getResponseMessage());
