...
function assureitITS() {
}

assureitITS.deHighlightPreviousElement = function () {
    Util.deghlightElement(previousElement);
};

assureitITS.sendSpiedObject = function (element, flag) {
    var data = {};
    data.type = "Object";
...
assureitITS.sendHealResult = function (data) {
    data.action = "heal";
    sendValues(JSON.stringify(data));
};

assureitITS.sendRecordedObject = function (element, method, input) {
    var data = {};
    data.action = "record";
    data.method = method;
    data.input = input;
...
assureitITS.getFrameValue = function () {
    return getFrameData();
};
